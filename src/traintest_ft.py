...
def train(audio_model, train_loader, test_loader, args):
    ...
    progress = []
    best_epoch, best_mse, best_mae = 0, np.inf, np.inf
    global_step, epoch = 0, 0
    start_time = time.time()
    exp_dir = args.exp_dir

    def _save_progress():
        progress.append([epoch, global_step, best_epoch, best_mse, time.time() - start_time])
    ...
    mlp_params = list(filter(lambda kv: kv[0] in mlp_list, audio_model.named_parameters()))
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, labels) in enumerate(train_loader):
            ...
            print(f"model ouutput: {audio_output}")
        ...
        result[epoch-1, :] = [mse, mae, r2, optimizer.param_groups[0]['lr']]
